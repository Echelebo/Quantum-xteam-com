					...
					<div class="col-lg-4 p-3 rounded card bg-{{$bg}}">
							...
							<h2 class="card-title mb-3 text-{{$text}}"> {{$method->name}}</h2>
								...
								<div class="text-center">
									<a class="btn" href="#" data-toggle="modal" data-target="#withdrawalModal{{$method->id}}" style=" background-color: #6495ED; color: #ffffff;"><i class="fa fa-plus"></i> Request withdrawal</a>
								</div>
								...
								<div class="modal-content">
								  ...
								  <div class="modal-body bg-{{$bg}}">
										<form style="padding:3px;" role="form" method="post" action="{{action('SomeController@withdrawal')}}">
											  ...
											  <input  class="form-control text-{{$text}} bg-{{$bg}} " style="color:#FF7742;" value="{{$method->name}}" type="text" disabled><br/>
											   <input value="{{$method->name}}" type="hidden" name="payment_mode">
											   <input value="{{$method->id}}" type="hidden" name="method_id"><br/>
											   
											   <input type="hidden" name="_token" value="{{ csrf_token() }}">
											   <input type="submit" class="btn btn-primary" value="Submit" onclick="this.disabled=true;this.value='processing, please wait...';this.form.submit();" />
									   </form>
								  </div>
								</div>
							  </div>
