		...
		<div class="main-panel bg-{{$bg}}">
			<div class="content bg-{{$bg}}">
				<div class="page-inner">
					<div class="mt-2 mb-4">
						<h1 class="title1 text-{{$text}}">Re-Invest</h1>
					</div>
						...
						<div class="col-lg-6 p-4 card bg-{{$bg}} shadow-lg">
							...
							<div class="pricing-table purple border bg-{{$bg}} shadow-lg">
								...
								<p align="center"><font color="#6495ED" size="5">
							       ...
							       $c = $b-$a;
							       echo $c."%";
							       ?> in {{$plan->increment_interval}}</font></p>
								...
								<div class="">
									<form style="padding:3px;" role="form" method="post" action="{{action('Controller@joinplan')}}">
										...
										<input type="hidden" name="inv_increment" value="{{$plan->increment_amount}}">
										<input type="hidden" name="id" value="{{ $plan->id }}">
										<input type="hidden" name="_token" value="{{ csrf_token() }}">
										<input type="submit"  onclick="this.disabled=true;this.value='processing, please wait...';this.form.submit();" class="btn btn-block pricing-action nav-pills" style="background-color: #6495ED; color: #ffffff; border-radius: 40px;" value="Invest">
									</form>
								</div>
