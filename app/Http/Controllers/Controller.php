    ...
    */
	public function dashboard(Request $request)
    {
        ...
        if($request->session()->has('ref_by')) {
           ...
           $dref=users::where('id',$ref_by)->first();
           $user=users::all();
        ...
        $objDemo->message = "This is to inform you that a user have registered with your referral link.";
        $objDemo->sender = "$settings->site_name";
        $objDemo->date = \Carbon\Carbon::Now();
        $objDemo->subject = "Referred User!";
            
        Mail::bcc($dref->email)->send(new NewNotification($objDemo));
    ...
    if($user->account_bal < $plan_price){
        ...
        return redirect()->route('reinvest')
      ->with('message', 'Your account balance is insufficient to invest this amount.');
      ...
      if($plan->type=='Main'){
        ...
        //create history
          ...
          $userplanid = DB::table('user_plans')->insertGetId(
            ...
            'inv_increment'=>$request['inv_increment'],
        ...
        //calculate top up earnings and
          ...
          foreach($plans as $plan){
              ...
              if($settings->trade_mode=='on'){
                  ...
                  if($dplan->increment_interval=="31Days"){
                  $togrow=\Carbon\Carbon::now()->subDays(31)->toDateTimeString();
                  $dtme = $plan->last_growth->diffInDays();
                }elseif($dplan->increment_interval=="21Days"){
                  $togrow=\Carbon\Carbon::now()->subDays(21)->toDateTimeString();
                  $dtme = $plan->last_growth->diffInDays();
                }elseif($dplan->increment_interval=="7Days"){
                  $togrow=\Carbon\Carbon::now()->subDays(7)->toDateTimeString();
                  $dtme = $plan->last_growth->diffInDays();
                }elseif($dplan->increment_interval=="5Days"){
                  $togrow=\Carbon\Carbon::now()->subDays(5)->toDateTimeString();
                  $dtme = $plan->last_growth->diffInDays();
                }elseif($dplan->increment_interval=="168Hours"){
                  $togrow=\Carbon\Carbon::now()->subHours(168)->toDateTimeString();
                  $dtme = $plan->last_growth->diffInHours();
                }elseif($dplan->increment_interval=="120Hours"){
                  $togrow=\Carbon\Carbon::now()->subHours(120)->toDateTimeString();
                  $dtme = $plan->last_growth->diffInHours();
                }elseif($dplan->increment_interval=="72Hours"){
                  $togrow=\Carbon\Carbon::now()->subHours(72)->toDateTimeString();
                  $dtme = $plan->last_growth->diffInHours();
                }elseif($dplan->increment_interval=="48Hours"){
                  $togrow=\Carbon\Carbon::now()->subHours(48)->toDateTimeString();
                  $dtme = $plan->last_growth->diffInHours();
                }elseif($dplan->increment_interval=="24Hours"){
                  $togrow=\Carbon\Carbon::now()->subHours(24)->toDateTimeString();
                  $dtme = $plan->last_growth->diffInHours();
                }
                
                //expiration
                
                if($plan->inv_duration=="31Days"){
                  $condition=$plan->activated_at->diffInDays() < 31 && $user->trade_mode=="on";
                  $condition2=$plan->activated_at->diffInDays() >= 31;
                }elseif($plan->inv_duration=="21Days"){
                  $condition=$plan->activated_at->diffInDays() < 21 && $user->trade_mode=="on";
                  $condition2=$plan->activated_at->diffInDays() >= 21;
                }
                elseif($plan->inv_duration=="7Days"){
                  $condition=$plan->activated_at->diffInDays() < 7 && $user->trade_mode=="on";
                  $condition2=$plan->activated_at->diffInDays() >= 7;
                }elseif($plan->inv_duration=="168Hours"){
                  $condition=$plan->activated_at->diffInHours() < 168 && $user->trade_mode=="on";
                  $condition2=$plan->activated_at->diffInHours() >= 168;
                }elseif($plan->inv_duration=="120Hours"){
                  $condition=$plan->activated_at->diffInHours() < 120 && $user->trade_mode=="on";
                  $condition2=$plan->activated_at->diffInHours() >= 120;
                }elseif($plan->inv_duration=="72Hours"){
                  $condition=$plan->activated_at->diffInHours() < 72 && $user->trade_mode=="on";
                  $condition2=$plan->activated_at->diffInHours() >= 72;
                }
                elseif($plan->inv_duration=="48Hours"){
                  $condition=$plan->activated_at->diffInHours() < 48 && $user->trade_mode=="on";
                  $condition2=$plan->activated_at->diffInHours() >= 48;
                }
                elseif($plan->inv_duration=="24Hours"){
                  $condition=$plan->activated_at->diffInHours() < 24 && $user->trade_mode=="on";
                  $condition2=$plan->activated_at->diffInHours() >= 24;
                }
